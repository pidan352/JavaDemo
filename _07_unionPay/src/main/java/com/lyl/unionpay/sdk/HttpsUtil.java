...
/*	static {//商户如果通过代理访问需要的改动点
    ...
    public static Response send(String urlStr, byte[] data, Map<String, String> reqHeader, String requestMethod) {
...
//            }
            ...
            for (String key : map.keySet()) {
                ...
                for (int i = 1; i < value.size(); i++) {
                    strVal = strVal + "; " + value.get(i);
                }
                if (key != null) {
                    resp.headers.put(key, strVal);
                }
            }
            ...
            if (httpCode != HttpURLConnection.HTTP_OK) {
                ...
                resp.body = respBody;
                if (respBody != null) {
                    log.error("httpCode=" + httpCode + ", body: [" + new String(respBody, encoding) + "]");
                } else {
                    log.error("httpCode=" + httpCode + ", body is null");
                }
            } else {
                respBody = read(httpURLConnection.getInputStream());
                resp.body = respBody;
                log.debug("resp headers: " + resp.headers + ", body: [" + new String(respBody, encoding) + "]");
            }
            return resp;
        } catch (Exception e) {
    ...
    public static byte[] post(String url, byte[] data, Map<String, String> reqHeader) {
        Response response = send(url, data, reqHeader, "POST");
        return response.body;
    }

    public static Response postForResponse(String url, byte[] data, Map<String, String> reqHeader) {
    ...
    public static byte[] get(String url, Map<String, String> reqHeader) {
        Response response = send(url, null, reqHeader, "GET");
        return response.body;
    }

    public static byte[] get(String url) {
        Response response = send(url, null, new TreeMap<String, String>() {
