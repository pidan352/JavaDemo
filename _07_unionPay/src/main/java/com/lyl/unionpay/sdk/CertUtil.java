...
import static com.lyl.unionpay.sdk.SDKUtil.isEmpty;
...
public class CertUtil {
    ...
    public static X509Certificate readX509Cert(InputStream is) {
        X509Certificate cert = null;
        CertificateFactory cf = null;
        try {
            cf = CertificateFactory.getInstance("X.509", "BC");
    ...
    public static int resetEncryptCertPublicKey(String strCert) {
        ...
        if (!f.exists()) {
            ...
            // 将本地证书进行备份存储
            int i = localCertPath.lastIndexOf(SDKConstants.POINT);
